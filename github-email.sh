...
curl "https://api.github.com/users/$user/events?access_token=$GH_EMAIL_TOKEN" -s \
    | sed -nE 's#^.*"(email)": "([^"]+)",.*$#\2#p' \
    | sort -u
...
if [[ -z $repo ]]; then
    # get all owned repos
    repo="$(curl "https://api.github.com/users/$user/repos?type=owner&sort=updated&access_token=$GH_EMAIL_TOKEN" -s \
        | sed -nE 's#^.*"name": "([^"]+)",.*$#\1#p' \
        | head -n1)"
fi

curl "https://api.github.com/repos/$user/$repo/commits?access_token=$GH_EMAIL_TOKEN" -s \
